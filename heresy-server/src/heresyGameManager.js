...
reconnect(c,p){this.requirePlayer(c,p);this.db.prepare('UPDATE hr_players SET connected=1 WHERE game_code=? AND player_code=?').run(c,p);return this.state(c,p);}
  ...
  resolveDayVote(g){
    ...
    // reset on any gap).
    const targetCode=leaderCodes[0],targetName=this.player(c,targetCode)?.name||'someone',targetVotes=votes.filter(v=>v.choice===targetCode),voterNames=targetVotes.map(v=>this.player(c,v.voter_code)?.name||'?').filter(Boolean),threshold=Math.ceil(totalAlive*0.6),markedForEscalation=!!this.player(c,targetCode)?.interrogated_before,outcome=(top>=threshold||markedForEscalation)?'lynch':'interrogate';
