...
reconnect(c,p){this.requirePlayer(c,p);this.db.prepare('UPDATE hr_players SET connected=1 WHERE game_code=? AND player_code=?').run(c,p);return this.state(c,p);}
  ...
  resolveDayVote(g){
    ...
    // Tiered Lynch (tiered-lynch.md v1.0.0): supersedes the old consecutive-
    // day escalation entirely. Outcome is decided same-day by what fraction
    // of LIVING votes the leader cleared — no cross-day memory, no target
    // streak. >=60% executes outright; any positive count below that
    // interrogates instead, repeatably, with no cap on how many times the
    // same target can be probed.
    const targetCode=leaderCodes[0],targetName=this.player(c,targetCode)?.name||'someone',targetVotes=votes.filter(v=>v.choice===targetCode),voterNames=targetVotes.map(v=>this.player(c,v.voter_code)?.name||'?').filter(Boolean),threshold=Math.ceil(totalAlive*0.6),outcome=top>=threshold?'lynch':'interrogate';
