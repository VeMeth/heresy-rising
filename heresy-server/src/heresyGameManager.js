...
reconnect(c,p){this.requirePlayer(c,p);this.db.prepare('UPDATE hr_players SET connected=1 WHERE game_code=? AND player_code=?').run(c,p);return this.state(c,p);}
  ...
  setPhase(c,phase,round,dayStage=null){const g=this.game(c),duration=phase==='night'?g.night_ms:g.day_ms,deadline=this.now()+duration,stage=phase==='day'?(dayStage||'vote'):dayStage;this.db.prepare('UPDATE hr_games SET phase=?,round=?,day_stage=?,deadline=?,updated_at=? WHERE code=?').run(phase,round,stage,deadline,this.now(),c);if(phase==='night')this.db.prepare('UPDATE hr_players SET confession_token_round=NULL WHERE game_code=?').run(c);if(phase==='day'){const votingEnabled=round>=this.config.rules.day.FIRST_VOTING_ROUND;this.system(c,votingEnabled?`Day ${round}: vote for a target or stand down.`:`Day ${round} begins — no vote today. Introduce yourself and observe.`);}
  ...
  else if(phase==='day'&&round>=this.config.rules.day.FIRST_VOTING_ROUND)for(const p of aliveBots){const targets=this.players(c).filter(x=>x.alive&&x.player_code!==p.player_code).map(x=>x.player_code);this.emitBotPrompt(c,{kind:'day_vote_prompt',playerCode:p.player_code,round,votingEnabled:true,deadline,legalTargets:targets});}}
  resolveNight(g){const c=g.code,players=this.players(c).filter(p=>p.alive),totalPlayers=this.players(c).length,actions=this.actions(c,g.round),traps=new Map(actions.filter(a=>a.kind==='boobytrap').map(a=>[a.target_code,a]));const protectedIds=new Set(),bodyguards=new Map();
  ...
  applyTorture(g,targetCode){const c=g.code,target=this.player(c,targetCode);if(!target?.alive){this.system(c,'Vote tally — The leading candidate was already dead; the conclave dispersed without judgement.');return this.skipDay(g,'invalid-target');}
    ...
    if(tier>=2){
      this.db.prepare('UPDATE hr_players SET cripple_tier=2,alive=0 WHERE game_code=? AND player_code=?').run(c,targetCode);
      ...
      this.system(c,`${targetDisplay} was ${roleName} (${target.faction}).`);
      this.emitAnnouncement(c,{type:'torture-death',title:'INTERROGATION DEATH',message:`${targetDisplay} was tortured and expired under interrogation, revealing ${roleName}.`,victim:{name:targetDisplay,role:roleName,faction:target.faction},round:g.round,phase:'day'});
  ...
  respondTorture(c,p,response){const g=this.requireGame(c),target=this.player(c,p);if(g.phase!=='day'||g.day_stage!=='response'||g.last_tortured_target!==p)throw new Error('No response is pending');if(target.cripple_tier>=this.config.rules.cripple.DIRECT_ASK_AT_TIER)throw new Error('Tier 3 requires a direct ask');if(!['confess','resist','refuse-break'].includes(response))throw new Error('Invalid response');if(response==='confess'){
      ...
      this.db.prepare('UPDATE hr_players SET confessed=1,confession_token_round=?,skip_next_night=1 WHERE game_code=? AND player_code=?').run(g.round,c,p);const targetDisplay=this.displayName(g,target);this.system(c,`${targetDisplay} confessed: ${this.role(target.role_id).displayName}.`);this.emitAnnouncement(c,{type:'confession',title:'CONFESSION',message:`${targetDisplay} confessed: ${this.role(target.role_id).displayName}.`,victim:{name:targetDisplay},round:g.round,phase:'day'});}else if(response==='resist')this.changeDrift(c,p,this.config.drift.RESIST,'resist');else{this.db.prepare('UPDATE hr_players SET cripple_tier=MAX(cripple_tier,?),skip_next_night=1 WHERE game_code=? AND player_code=?').run(this.config.rules.cripple.REFUSE_BREAK_TIER,c,p);this.changeDrift(c,p,this.config.drift.REFUSE_BREAK,'refuse-break');}this.applyHereticCap(c,g.round);if(!this.finishIfWon(c))this.setPhase(c,'night',g.round);return this.state(c,p);}
  askConfession(c,asker,targetCode){const g=this.requireGame(c);this.requireAlive(c,asker);const target=this.requirePlayer(c,targetCode);if(g.phase!=='day'||g.day_stage!=='response'||targetCode!==g.last_tortured_target||target.cripple_tier<this.config.rules.cripple.DIRECT_ASK_AT_TIER)throw new Error('This player cannot be asked now');
  ...
  vote(c,p,choice,justification=''){const g=this.requireGame(c),v=this.requireAlive(c,p),cleanChoice=String(choice||'');if(g.phase!=='day'||g.day_stage==='response'||g.round<this.config.rules.day.FIRST_VOTING_ROUND)throw new Error('Voting is closed');
