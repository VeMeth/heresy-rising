...
reconnect(c,p){this.requirePlayer(c,p);this.db.prepare('UPDATE hr_players SET connected=1 WHERE game_code=? AND player_code=?').run(c,p);return this.state(c,p);}
  ...
  askConfession(c,asker,targetCode){const g=this.requireGame(c);this.requireAlive(c,asker);const target=this.requirePlayer(c,targetCode);if(g.phase!=='day'||g.day_stage!=='response'||targetCode!==g.last_tortured_target||target.cripple_tier<this.config.rules.cripple.DIRECT_ASK_AT_TIER)throw new Error('This player cannot be asked now');
    const targetDisplay=this.displayName(g,target),roleName=target.role_id?this.role(target.role_id).displayName:'Unknown';this.db.prepare('UPDATE hr_players SET confessed=1,alive=0 WHERE game_code=? AND player_code=?').run(c,targetCode);this.system(c,`${targetDisplay} is forced to confess: ${roleName}.`);this.system(c,`${targetDisplay}'s body breaks. They expire under the weight of their confession.`);this.emitAnnouncement(c,{type:'confession-death',title:'FORCED CONFESSION',message:`${targetDisplay} is forced to confess: ${roleName}. Their body cannot withstand the interrogation.`,victim:{name:targetDisplay,role:roleName,faction:target.faction},round:g.round,phase:'day'});for(const p of this.players(c).filter(x=>x.alive))this.changeDrift(c,p.player_code,this.config.drift.WITNESSED_VIOLENCE,'witnessed-violence');this.applyHereticCap(c,g.round);if(!this.finishIfWon(c))this.setPhase(c,'night',g.round);return this.state(c,asker);}
