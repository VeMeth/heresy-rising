...
reconnect(c,p){this.requirePlayer(c,p);this.db.prepare('UPDATE hr_players SET connected=1 WHERE game_code=? AND player_code=?').run(c,p);return this.state(c,p);}
  ...
  resolveDayVote(g){
    ...
    // Tiered Lynch (tiered-lynch.md v1.1.0): outcome is decided same-day by
    // what fraction of LIVING votes the leader cleared — >=60% executes
    // outright. v1.1.0 adds a second, narrower kill path: if the SAME
    // suspect was ALSO yesterday's leader AND yesterday's outcome was an
    // interrogation (not a kill, not a skip), today's interrogation
    // escalates to a kill too — one probe cycle is all the cabal gets to
    // defend a suspect. Reuses previousDayResolution (added for the T1-
    // recovery/repeatable-interrogation fix) rather than new state: a skip
    // or kill day already writes an outcome other than 'interrogate', so
    // the streak breaks for free with no explicit reset needed.
    const targetCode=leaderCodes[0],targetName=this.player(c,targetCode)?.name||'someone',targetVotes=votes.filter(v=>v.choice===targetCode),voterNames=targetVotes.map(v=>this.player(c,v.voter_code)?.name||'?').filter(Boolean),threshold=Math.ceil(totalAlive*0.6),previous=this.previousDayResolution(c,g.round),consecutiveInterrogation=previous?.outcome==='interrogate'&&previous.target===targetCode,outcome=(top>=threshold||consecutiveInterrogation)?'lynch':'interrogate';
