...
reconnect(c,p){this.requirePlayer(c,p);this.db.prepare('UPDATE hr_players SET connected=1 WHERE game_code=? AND player_code=?').run(c,p);return this.state(c,p);}
  ...
  resolveNight(g){const c=g.code,players=this.players(c).filter(p=>p.alive),totalPlayers=this.players(c).length,actions=this.actions(c,g.round),traps=new Map(actions.filter(a=>a.kind==='boobytrap').map(a=>[a.target_code,a]));const protectedIds=new Set(),bodyguards=new Map();
    ...
    for(const p of players){const a=actions.find(x=>x.actor_code===p.player_code),role=this.role(p.role_id);if(p.skip_next_night){this.db.prepare('UPDATE hr_players SET skip_next_night=0 WHERE game_code=? AND player_code=?').run(c,p.player_code);continue;}if(!a){this.changeDrift(c,p.player_code,this.config.drift.NIGHTLY_SLEEP_RECOVERY,'sleep');continue;}if(a.kind==='kill'||a.kind==='blood-ritual'||a.kind==='possess')continue;const cost=role.scaledCostKey?resolveScaledCost(this.config.drift.scaledCosts,role.scaledCostKey,'t'+(Number(String(a.variant||'T1').replace('T',''))||1),totalPlayers):role.driftWeight;this.changeDrift(c,p.player_code,cost,'night-action');}
