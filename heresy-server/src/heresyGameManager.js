...
reconnect(c,p){this.requirePlayer(c,p);this.db.prepare('UPDATE hr_players SET connected=1 WHERE game_code=? AND player_code=?').run(c,p);return this.state(c,p);}
  ...
  resolveNight(g){const c=g.code,players=this.players(c).filter(p=>p.alive),actions=this.actions(c,g.round),traps=new Map(actions.filter(a=>a.kind==='boobytrap').map(a=>[a.target_code,a]));const protectedIds=new Set(),bodyguards=new Map();
    ...
    for(const sv of actions.filter(x=>x.kind==='blood-ritual')){const attacker=this.player(c,sv.actor_code);if(!attacker?.alive)continue;this.changeDrift(c,sv.actor_code,this.config.drift.BLOOD_RITUAL_ATTACKER_COST,'night-action');if(traps.has(sv.target_code))this.changeDrift(c,sv.actor_code,this.config.drift.TRAP_DRIFT,'trap');
      ...
      if(escalate){if(victim){this.db.prepare('UPDATE hr_players SET cripple_tier=3,alive=0 WHERE game_code=? AND player_code=?').run(c,victim);const victimName=this.player(c,victim).name;const slainBody=this.flavor('slain',{victim:victimName});this.system(c,slainBody);this.emitAnnouncement(c,{type:'kill',title:'SLAIN IN THE NIGHT',message:slainBody,victim:{name:victimName},round:g.round,phase:'night'});for(const witness of this.players(c).filter(x=>x.alive))this.changeDrift(c,witness.player_code,this.config.drift.WITNESSED_VIOLENCE,'witnessed-violence');}this.event(c,'blood-ritual',{round:g.round,attacker:sv.actor_code,target:sv.target_code,outcome:'kill',landed:!!victim});}
  ...
  resolveDayVote(g){
    ...
    if(leaderCodes.length>1){const maxDrift=Math.max(...leaderCodes.map(code=>this.player(c,code)?.drift??-1));leaderCodes=leaderCodes.filter(code=>(this.player(c,code)?.drift??-1)===maxDrift);if(leaderCodes.length!==1){const leaderNames=leaderCodes.map(code=>this.player(c,code)?.name).filter(Boolean);this.system(c,`Vote tally — Tied between ${leaderNames.join(', ')} at ${top} votes; drift could not break it. The conclave dispersed.`);return this.skipDay(g,'tied-drift');}}
    const targetCode=leaderCodes[0],targetName=this.player(c,targetCode)?.name||'someone',targetVotes=votes.filter(v=>v.choice===targetCode),voterNames=targetVotes.map(v=>this.player(c,v.voter_code)?.name||'?').filter(Boolean),previous=this.previousDayResolution(c,g.round),samePrevious=previous?.outcome==='interrogate'&&previous.target===targetCode&&this.player(c,targetCode)?.alive,outcome=samePrevious?'lynch':'interrogate';
  ...
  applyInterrogation(g,targetCode){const c=g.code,target=this.player(c,targetCode);if(!target?.alive){this.system(c,'Vote tally — The leading candidate was already dead; the conclave dispersed without judgement.');return this.skipDay(g,'invalid-target');}if(target.cripple_tier>=3){this.system(c,`Vote tally — ${target.name} is already ${crippleSeverityLabel(3)} and cannot be interrogated again; the conclave dispersed without judgement.`);return this.skipDay(g,'target-already-crippled');}const tier=Math.min(3,(Number(target.cripple_tier)||0)+1),voters=this.dayVotes(c,g.round).filter(v=>v.choice===targetCode);this.db.prepare('UPDATE hr_games SET last_interrogated_target=?,last_interrogation_tier=?,day_stage=?,deadline=NULL,updated_at=? WHERE code=?').run(targetCode,tier,null,this.now(),c);this.db.prepare('UPDATE hr_players SET cripple_tier=?,tier1_until_round=? WHERE game_code=? AND player_code=?').run(tier,tier===1?g.round:null,c,targetCode);const voterResults=voters.map(v=>({voter:v.voter_code,votedFor:v.choice,driftDelta:0}));let witnessedDrift=0;if(target.faction==='loyalist'){for(const result of voterResults){result.driftDelta=1;this.changeDrift(c,result.voter,1,'wrong-interrogation');}}else if(target.faction==='heretic'){witnessedDrift=-1;for(const p of this.players(c).filter(x=>x.alive))this.changeDrift(c,p.player_code,-1,'right-interrogation-cleansing');}const payload={round:g.round,outcome:'interrogate',target:targetCode,voterResults,witnessedDrift,alignmentRevealed:null,crippleTier:tier};this.event(c,'day-resolution',payload);const tortureBody=this.flavor('tortureChamber',{victim:target.name,tier,severity:crippleSeverityLabel(tier)});this.system(c,tortureBody);this.emitAnnouncement(c,{type:'torture-chamber',title:'TORTURE CHAMBER',message:tortureBody,victim:{name:target.name},round:g.round,phase:'day'});this.applyHereticCap(c,g.round);if(!this.finishIfWon(c))this.setPhase(c,'night',g.round);return payload;}
  applyLynch(g,targetCode){const c=g.code,target=this.player(c,targetCode),votes=this.dayVotes(c,g.round).filter(v=>v.choice===targetCode),voterResults=votes.map(v=>({voter:v.voter_code,votedFor:v.choice,driftDelta:0}));this.db.prepare('UPDATE hr_players SET cripple_tier=3,alive=0 WHERE game_code=? AND player_code=?').run(c,targetCode);this.system(c,`${target.name} was lynched and revealed ${target.faction}.`);this.emitAnnouncement(c,{type:'lynch',title:'SENTENCE EXECUTED',message:`${target.name} was lynched and revealed as ${target.faction}.`,victim:{name:target.name,faction:target.faction},round:g.round,phase:'day'});for(const p of this.players(c).filter(x=>x.alive))this.changeDrift(c,p.player_code,1,'witnessed-violence');if(target.faction==='loyalist')for(const result of voterResults){result.driftDelta=this.config.drift.WRONG_LYNCH;this.changeDrift(c,result.voter,this.config.drift.WRONG_LYNCH,'wrong-lynch');}const payload={round:g.round,outcome:'lynch',target:targetCode,voterResults,witnessedDrift:1,alignmentRevealed:target.faction,crippleTier:3};this.event(c,'day-resolution',payload);this.applyHereticCap(c,g.round);if(!this.finishIfWon(c))this.setPhase(c,'night',g.round);return payload;}
...
if(action.kind==='forgery')return this.forge(c,p,asPlayerCode,body);const target=this.requireAlive(c,targetCode);if(action.target==='other'&&!canProtectSelf(role.id)&&targetCode===p)throw new Error('Choose another player');if(action.kind==='protect'&&!validateRotation(this.db,c,p,targetCode,g.round))throw new Error('Cannot protect the same target on consecutive nights');if(action.kind==='bodyguard'&&!validateRotation(this.db,c,p,targetCode,g.round))throw new Error('Cannot proxy the same target on consecutive nights');if((action.target==='hostile'||action.kind==='kill')&&!isHostileTo(actor,target))throw new Error('Target is not hostile');if(action.kind==='possess'&&target.possessed_by)throw new Error('That player is already possessed');if(action.variants&&!action.variants.includes(variant))throw new Error('Invalid action variant');if(['sermon','corrupt-sermon'].includes(action.kind)){const s=this.config.drift.sermons[variant],uses=this.usage(c,p,variant);if(s.limit!==null&&uses>=s.limit)throw new Error('Sermon limit reached');
    ...
    this.db.prepare('INSERT INTO hr_actions VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(game_code,round,actor_code) DO UPDATE SET kind=excluded.kind,target_code=excluded.target_code,variant=excluded.variant,data=excluded.data,created_at=excluded.created_at').run(c,g.round,p,action.kind,targetCode,variant,data?JSON.stringify(data):null,this.now());if(g.phase==='night'&&actor.faction==='heretic'){const cabalLabel=action.kind==='kill'?`marked ${target.name} for death`:action.kind==='boobytrap'?`set a trap on ${target.name}`:action.kind==='corrupt-sermon'?`targets ${target.name} with a corrupt sermon`:action.kind==='heretical-catalyst'?`invokes the catalyst on ${target.name}`:action.kind==='possess'?`stirs the Warp toward ${target.name}`:`uses ${action.kind}`;this.factionSystem(c,`Cabalite ${actor.name} ${cabalLabel}.`);}return {kind:action.kind,targetCode,variant};}
  ...
  sendMessageAs(c,p,body){const g=this.game(c),actor=this.requireAlive(c,p);this.authorizeChannel(g,actor,'public',true);const target=this.players(c).find(x=>x.possessed_by===p&&x.alive);if(!target)throw new Error('You are not possessing anyone right now');return this.insertMessage(c,'public',null,p,target.name,String(body||'').trim().slice(0,1000),'player');}
