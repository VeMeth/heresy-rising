...
export function createHeresyServer({ databasePath, now } = {}) {
  ...
  app.delete('/api/admin/games/:code',requireAdmin,async (req,res)=>{try{const code=normalizeRoomCode(req.params.code);const result=gameManager.adminDeleteGame(code);// Clean up any bots that were in this conclave.try{await fetch(`${config.botManager.url}/bots/by-conclave/${encodeURIComponent(code)}`,{method:'DELETE',headers:{'Authorization':`Bearer ${config.botManager.adminApiKey}`,'Content-Type':'application/json'}}).catch(()=>{});}catch{}// bot-manager may be down; ignoreres.json(result);}catch(e){res.status(400).json({error:e.message});}});
