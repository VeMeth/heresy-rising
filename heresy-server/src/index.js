...
export function createHeresyServer({ databasePath, now } = {}) {
  ...
  function broadcast(code,event='game:state'){for(const socket of io.sockets.sockets.values()){if(!socket.rooms.has(code)||!socket.data.playerCode)continue;try{const isMember=!!gameManager.player(code,socket.data.playerCode);const state=isMember?gameManager.state(code,socket.data.playerCode):gameManager.spectate(code);socket.emit(event,{state});if(state.status==='ended')socket.emit('game:ended',{state});}catch{}}}
  function broadcastMessage(code,message){for(const client of io.sockets.sockets.values()){try{if(!client.rooms.has(code)||!client.data.playerCode)continue;const player=gameManager.player(code,client.data.playerCode);if(!player){if(message.channel==='public')client.emit('chat:message',{message});continue;}if(message.channel==='public'||(message.channel==='faction'&&player.faction==='heretic')||(message.channel==='graveyard'&&!player.alive)||(message.channel==='private'&&message.recipient_code===client.data.playerCode))client.emit('chat:message',{message});}catch{}}}
