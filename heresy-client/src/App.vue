...
async function startGame(composition) { try { await command('game:start', { code: game.value.code, setup: { maxDrift: game.value.setup?.maxDrift || game.value.maxDrift, dayMs: game.value.setup?.dayMs || game.value.dayMs, nightMs: game.value.setup?.nightMs || game.value.nightMs, ...(composition ? { composition } : {}) } }); compositionErrors.value = []; messagesByChannel.value = { public: [], faction: [], graveyard: [] }; hasMoreByChannel.value = { public: true, faction: true, graveyard: true }; await loadHistory(); } catch (e) { if (e.data?.errors) { compositionErrors.value = e.data.errors; error.value = ''; toast.value = ''; } } }
...
async function loadHistory(before) { try { const data = await command('chat:history', { code: game.value.code, playerCode: profile.value?.playerCode, channel: channel.value, before, limit: 50 }); if (before == null) { messagesByChannel.value = { ...messagesByChannel.value, [channel.value]: data?.messages || [] }; } else { mergeMessages(channel.value, data?.messages || [], true); } hasMoreByChannel.value = { ...hasMoreByChannel.value, [channel.value]: !!data?.hasMore }; } catch {} }
