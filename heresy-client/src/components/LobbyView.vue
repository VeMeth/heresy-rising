<template>
  <section class="lobby page">
    ...
    <div class="lobby-actions"><button class="secondary" :class="{selected:me?.ready}" :disabled="busy" @click="$emit('ready')">{{ me?.ready?'Stand down':'Mark ready' }}</button><button v-if="isHost" class="primary" :disabled="!canStart||busy" @click="$emit('start')">Seal the chamber</button><span v-else>Waiting for the cell commander.</span></div>
  </section>
</template>
<script setup>
