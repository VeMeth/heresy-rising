<template>
  <section class="game-page">
    ...
    <div class="game-grid">
      ...
      <section class="panel chat-panel" :class="{'mobile-hidden':mobileTab!=='chat'}"><div class="channel-tabs"><button v-for="c in channels" :key="c.id" :class="{active:channel===c.id}" @click="$emit('channel',c.id)">{{ c.label }}<small>{{ c.note }}</small></button></div><button v-if="messages.length && hasMore" class="load-history" @click="$emit('history',messages[0]?.id)">Load earlier transmissions</button><div ref="feed" class="message-feed"><div v-if="!messages.length" class="empty-chat"><strong>No transmissions recorded</strong></div><button v-if="splitMessages.earlier.length" type="button" class="earlier-toggle" @click="showEarlier=!showEarlier">{{ showEarlier?'Hide':'Show' }} earlier transmissions ({{ splitMessages.earlier.length }})</button><div v-show="showEarlier" class="earlier-day"><article v-for="m in splitMessages.earlier" :key="m.id" :class="['message',{system:m.kind==='system',vote:m.kind==='vote',faction:m.kind==='faction'}]"><span v-if="m.kind==='system'" class="system-line">{{ m.body }}</span><template v-else><span class="avatar mini">{{ initial(m.author) }}</span><div><header><strong>{{ m.author }}</strong><time>{{ formatTime(m.createdAt) }}</time></header><p>{{ m.body }}</p></div></template></article></div><article v-for="m in splitMessages.current" :key="m.id" :class="['message',{system:m.kind==='system',vote:m.kind==='vote',faction:m.kind==='faction'}]"><span v-if="m.kind==='system'" class="system-line">{{ m.body }}</span><template v-else><span class="avatar mini">{{ initial(m.author) }}</span><div><header><strong>{{ m.author }}</strong><time>{{ formatTime(m.createdAt) }}</time></header><p>{{ m.body }}</p></div></template></article></div><form class="composer" @submit.prevent="post"><textarea v-model.trim="draft" maxlength="1000" rows="2" :disabled="!canChat" :placeholder="canChat?'Transmit… (Enter to send, Shift+Enter for newline)':'Channel sealed'" @keydown.enter.exact.prevent="post"></textarea><button class="primary" :disabled="!draft||!canChat">Transmit</button></form></section>
...
watch(()=>props.channel,()=>showEarlier.value=false);
...
const channels=computed(()=>[{id:'public',label:'Conclave',note:'public'},...(props.me?.faction==='heretic'?[{id:'faction',label:'Cabal',note:'heretics'}]:[]),...(!props.me?.alive?[{id:'graveyard',label:'Graveyard',note:'dead'}]:[]),{id:'private',label:'Dossier',note:'personal'}]),canChat=computed(()=>props.game.phase!=='ended'&&(props.channel!=='public'||props.game.phase!=='night')&&(props.me?.alive||props.channel==='graveyard'));
