...
export function emitWithAck(event, payload) { return new Promise((resolve, reject) => { const timer = setTimeout(() => reject(new Error('The server did not answer in time.')), 15000); socket.emit(event, payload, response => { clearTimeout(timer); if (response?.ok === false) { const err = new Error(response.error || 'Request rejected'); err.data = response; reject(err); } else resolve(response?.data ?? response); }); }); }
